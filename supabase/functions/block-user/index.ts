...
Deno.serve(async (request) => {
  ...
  const targetId = blockedId.trim();

  if (targetId === user.id) {
    return jsonResponse({ error: "You can't block yourself." }, 400);
  }
  ...
  if (error) {
    console.error(error);
    return jsonResponse({ error: 'Unable to block this user.' }, 500);
  }

  return jsonResponse({ blocked: true }, 200);
});
